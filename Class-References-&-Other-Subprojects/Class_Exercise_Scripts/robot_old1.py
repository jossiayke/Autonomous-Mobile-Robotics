...
class Robot(object):
    ...
    f = open('robot_path.txt','a')
    
    dutyset = [('f', [70,70]), ('rev', [30,30]),('l', 90),('r',90)]
...
#     const_zone = [6.5,0]
    ...
    def __init__(self, name: str):
        ...
        file_handler = logging.FileHandler('gc_log.log')
    ...
    def _email_media(self):
        ...
        subject = 'test_practice_email_yosephK_ENPM701'
#         subject = f'{self._block_now} block picked up for transportation to construction area: ' + f_time
    ...
    def _pivot(self,deg_diff,steady = False):
        ...
        while True:
            try:
                ...
                self._path.append((round(self._x,2),round(self._y,2),round(self._yaw,2)))
            
                self._dutycycle[self._direction] = np.clip(self._dutycycle[self._direction], lower_dc, upper_dc)
                
                if abs(yaw_diff) >= 0.5:
                    
                    if (abs(yaw_diff-yaw_diff_old) < 5):
#                     if abs(yaw_diff) >= 0.5:
                    ...
                    # Save robots pose by saving its x, yaw position            
                    self._path.append((round(self._x,2),round(self._y,2),round(self._imu_serial(),2)))
    ...
    def _go2block(self, direction='f', eucl_disp=0, just_drive=False, steady=False):
        ...
        if not just_drive and not steady:
            ...
            self._logger.info(f"Distance estimated {self._travel}")
            self._logger.info(f"Driving slowly {self._travel-5} cms forward")
        elif just_drive:
...
#             self._logger.info(f"Percent reach: {prcnt}")
            
            if (pos_encoder % 150) == 0: 
            
                # Save robots pose by saving its x, yaw position
                self._yaw = self._imu_serial()
                x += self._encoder2feet(pos_encoder) * math.cos(math.radians(self._yaw))
                y += self._encoder2feet(pos_encoder) * math.sin(math.radians(self._yaw))
            
                self._path.append((round(self._x+x,2),round(self._y+y,2),round(self._yaw,2)))
...
#                 self._dutycycle[self._direction][0] += 0.1 * self.duty[self._direction][0]
            ...
            if (error <= thresh) and (direction in ['f','rev']):
                ...
                self._logger.info(f"x value {self._x} and y value {self._y}")
    ...
    def _go2(self,destination=(0,0),area=''):
        ...
        # self._x = self._path[-1][0]
        # self._y = self._path[-1][1]
        # self._yaw = self._path[-1][2]
        
        self._logger.info(f"Pose Now: X- {self._x} and Y- {self._y} and Yaw- {self._yaw}")
        # Compute the euclidean distance between current robot location and construction zone
        disp = math.sqrt((destination[0] - self._x)**2 + (destination[1] - self._y)**2)
        # Calculate the yaw angle difference between these two points using slope equation
        rad_diff = math.atan2(destination[0]-self._x, destination[1]-self._y)                
        ...
        if (target_angle > 180 and target_angle <=360):
...
#         deg_diff = (deg_diff + 180) % 360 - 180
    ...
    def _localize(self):
        ...
        self._yaw = self._imu_serial()
        x1 = delta_ft * math.cos(math.radians(self._yaw))
        y1 = delta_ft * math.sin(math.radians(self._yaw))
        ...
        delta_ft = self._cm2feet(self._depth)
        self._yaw = self._imu_serial()
        x2 = delta_ft * math.cos(math.radians(self._yaw))
        y2 = delta_ft * math.sin(math.radians(self._yaw))
                ...
                if (not self._end_game):
                    ...
                    while True:
                        ...
                        if self._success:
                            ...
                            self._logger.info("Localizing to get accurate location of robot")
                            self._localize()
                            ...
                            self._logger.info("No toy blocks found in current pose. Change pose and")
                            ...
                            if search > 1:
                                ...
                                self._logger.info(f"Robot can't find {self._block_now} block in time.")
                                self._logger.info("In pursuit of time, moving on to next block in queue")
                                break
                            search += 1
                            
            else:
                self._logger.info(f"Mission complete. Take robot back to home base")
